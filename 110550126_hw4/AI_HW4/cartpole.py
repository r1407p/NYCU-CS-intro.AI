...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        # Begin your code
        # TODO
        ans = np.linspace(lower_bound,upper_bound,num_bins+1)
        ans = ans[1:-1];
        return ans
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        # TODO
        max_val = np.max(self.qtable[next_state[0],next_state[1],next_state[2],next_state[3]])
        
        self.qtable[state[0],state[1],state[2],state[3],action] = self.qtable[state[0],state[1],state[2],state[3],action] + self.learning_rate*(reward+self.gamma*max_val-self.qtable[state[0],state[1],state[2],state[3],action])
        if done:
