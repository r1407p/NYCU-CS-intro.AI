...
class Agent():
    ...
    def learn(self):
        ...
        #print(state)
        #memory = self.buffer.sample(self.batch_size)
        state = torch.FloatTensor(np.asarray(state))
        action = torch.LongTensor(np.asarray(action)).view(self.batch_size, 1)
        reward = torch.IntTensor(np.asarray(reward)).view(self.batch_size, 1)
        next_state = torch.FloatTensor(np.asarray(next_state))
        done = torch.IntTensor(np.asarray(done)).view(self.batch_size, 1)
        not_done = ~done
        q_value  = self.evaluate_net(state).gather(1,action)
        
        q_next_value = self.target_net(next_state).detach()
        q_next_value = q_next_value.max(1)[0].view(self.batch_size, 1)
        target_q_value = reward + self.gamma * q_next_value
        target_q_value = target_q_value*not_done
    ...
    def check_max_Q(self):
        ...
        tensor = torch.FloatTensor(self.env.reset())
        temp = torch.unsqueeze(tensor,0)
        action_values = self.target_net(temp)
