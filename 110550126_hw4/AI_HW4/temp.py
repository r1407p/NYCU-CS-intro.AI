    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if done:
            ...
            self.qtable[state][action] = (1-self.learning_rate) * self.qtable[state][action] + \
                self.learning_rate * \
                (reward + self.gamma * self.check_max_Q(next_state))
        # End your code
        # You can add some conditions to decide when to save your table

        def simple_test(env, cur_qtable):
            testing_agent = Agent(env)
            testing_agent.qtable = cur_qtable
            rewards = []
            for _ in range(10):
                ...
                while True:
                    ...
                    next_state, reward, done, _ = testing_agent.env.step(
                        action)
                    count += reward
                    if done == True:
        ...
        if done:
            test_rewards = simple_test(self.env, self.qtable)
            global best_mean_rewards
            mean_test_rewards = np.mean(test_rewards)
            if mean_test_rewards > best_mean_rewards:
                best_mean_rewards = mean_test_rewards
