    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                prob = self.belief.getProb(i,j)
                self.belief.setProb(i, j, density * prob)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE
        #Re-weight
        particle_dict = collections.defaultdict(float)
        for row, col in self.particles:
            y = util.rowToY(row) 
            x = util.colToX(col)
            distance = math.sqrt((x-agentX)**2 +(y-agentY)**2)
            density = util.pdf(distance, Const.SONAR_STD, observedDist)
            prob = self.particles[(row, col)]
            particle_dict[(row,col)] = prob*density
        #Re-sample
        #print(self.particles)
        new_particle = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            new_particle[util.weightedRandomChoice(particle_dict)] += 1
    ...
    def elapseTime(self) -> None:
        ...
        for old_particle in self.particles:
            for _ in range(self.particles[old_particle]):
                new_particle[util.weightedRandomChoice(self.transProbDict[old_particle])]+=1
        self.particles = new_particle
        # END_YOUR_CODE
